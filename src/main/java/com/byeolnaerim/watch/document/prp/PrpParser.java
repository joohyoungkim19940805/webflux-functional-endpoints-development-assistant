...
public final class PrpParser {
	...
	public List<PrpRouteInfo> extractPrpRoutes(
	...
	private void addContainerRoutes(
		List<PrpRouteInfo> output, CtType<?> type, CtMethod<?> method, CtAnnotation<?> container
	) {
	...
	private void addRoute(
		List<PrpRouteInfo> output, CtType<?> type, CtMethod<?> method, CtAnnotation<?> annotation
	) {
	...
	private static void validateInteraction(
		String interaction, CtMethod<?> method
	) {
	...
	private PrpTypeInfo parseRequest(
		CtMethod<?> method, String interaction
	) {
		...
		for (CtParameter<?> parameter : method.getParameters()) {
			...
			if ("PrpContext".equals( simple ) || "com.byeolnaerim.prp.spring.PrpContext".equals( qualified ))
				continue;
			payloads.add( parameter );

		}

		if (payloads.size() != 1) { throw invalid( method, "must declare exactly one application payload parameter plus optional PrpContext" ); }

		CtParameter<?> payload = payloads.get( 0 );
		CtTypeReference<?> type = payload.getType();

		if ("REQUEST_CHANNEL".equals( interaction )) {
			if (! isPublisherType( type ))
				throw invalid( method, "REQUEST_CHANNEL payload must be Flux or Publisher" );
			type = unwrapFirstGeneric( type );

		} else if ("DATAGRAM".equals( interaction )) {
			if (! isDatagramType( type ))
				throw invalid( method, "DATAGRAM payload must be byte[], ByteBuffer, or RoutedDatagram" );

		} else if (isPublisherType( type )) { throw invalid( method, interaction + " payload must be a single value, not a Publisher" ); }
	...
	private PrpTypeInfo parseResponse(
		CtMethod<?> method, String interaction
	) {

		if ("FIRE_AND_FORGET".equals( interaction ) || "DATAGRAM".equals( interaction )) { return voidInfo( method.getFactory().Type().voidPrimitiveType() ); }

		CtTypeReference<?> type = method.getType();
		if (type == null)
			return voidInfo( method.getFactory().Type().voidPrimitiveType() );

		if ("REQUEST_RESPONSE".equals( interaction ) && isVoidType( type )) { throw invalid( method, "REQUEST_RESPONSE must return a response value or asynchronous response value" ); }

		if ("REQUEST_STREAM".equals( interaction ) || "REQUEST_CHANNEL".equals( interaction )) {

			if (! isStreamOutputType( type )) { throw invalid( method, interaction + " must return Flux, Publisher, Iterable, Collection, or an array" ); }
	...
	private static CtTypeReference<?> unwrapAsyncSingle(
		CtTypeReference<?> type
	) {
		...
		if (("Mono".equals( simple ) || "CompletionStage".equals( simple ) || "CompletableFuture".equals( simple ) || "Publisher".equals( simple )) && type.getActualTypeArguments() != null && ! type
	...
	private static CtTypeReference<?> unwrapStreamOutput(
		CtTypeReference<?> type
	) {
		...
		if (("Flux".equals( simple ) || "Mono".equals( simple ) || "Publisher".equals( simple ) || "Iterable".equals( simple ) || "Collection".equals( simple ) || "List".equals( simple ) || "Set"
			.equals( simple )) && type.getActualTypeArguments() != null && ! type.getActualTypeArguments().isEmpty()) {
			return type.getActualTypeArguments().get( 0 );
	...
	private static CtTypeReference<?> unwrapFirstGeneric(
		CtTypeReference<?> type
	) {

		if (type != null && type.getActualTypeArguments() != null && ! type.getActualTypeArguments().isEmpty()) { return type.getActualTypeArguments().get( 0 ); }

		return type;

	}

	private static boolean isPublisherType(
		CtTypeReference<?> type
	) {
	...
	private static boolean isStreamOutputType(
		CtTypeReference<?> type
	) {
		...
		if (type instanceof CtArrayTypeReference<?>)
			return true;
		String simple = type.getSimpleName();
		return isPublisherType( type ) || "Iterable".equals( simple ) || "Collection".equals( simple ) || "List".equals( simple ) || "Set".equals( simple );

	}

	private static boolean isDatagramType(
		CtTypeReference<?> type
	) {
	...
	private static boolean isVoidType(
		CtTypeReference<?> type
	) {
	...
	private static IllegalStateException invalid(
		CtMethod<?> method, String message
	) {

		return new IllegalStateException( "Invalid @PrpRoute method " + method.getDeclaringType().getQualifiedName() + "#" + method.getSimpleName() + ": " + message );

	}

	private static String stringValue(
		CtAnnotation<?> annotation, String name
	) {

		var value = annotation.getValue( name );
		if (value instanceof CtLiteral<?> literal && literal.getValue() != null)
	...
	private static String interactionValue(
		...
		String text;
		if (value instanceof CtFieldRead<?> fieldRead && fieldRead.getVariable() != null)
			text = fieldRead.getVariable().getSimpleName();
		else
			text = value.toString();
