...
public final class ProjectDefaults {
	...
	static {
		...
		String pkg = ProjectDefaults.class.getPackageName(); // ex) com.starbearing.watch
		...
		CLASSPATH_ROOT = detectClasspathRoot( projectRoot ).toString().replace( '\\', '/' );
		TRIGGER_FILE = (CLASSPATH_ROOT + "/.reloadtrigger").replaceAll( "/+", "/" );
	...
	private static Path detectClasspathRoot(
		Path projectRoot
	) {
